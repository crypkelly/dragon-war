...
class Fight_Screen(Screen):
    ...
    def enemy_attack(self):
        if self.ids.enemy_strength_bar.value and E.agility > 30:
            ...
            attack_pic= Animation(size_hint=(0,0))
            attack_pic.start(self.ids.enemy_attack_pic)
            if int(self.ids.player_health_lbl.text) <= 0:
                self.player_lose()
    ...
    def player_lose(self):
        self.update_enemy_fight_label("You lose! I am amazeballs!...",.3)
        Clock.unschedule(self.update_bar)
        Clock.unschedule(self.enemy_update_bar)
        Selected_Dragon.agility= 0
        Selected_Dragon.intellect= 0
        self.ids.strength_bar.value= 0
        self.ids.player_health_lbl.text= "100"
        E.agility= 0
        E.intellect= 0
        self.ids.enemy_strength_bar.value= 0
        self.ids.enemy_health_lbl.text = "100"
        Clock.schedule_once(self.arena_exit, 4)
        
    def enemy_lose(self, dt):
        self.update_fight_label("I win! I should probably call you an ambulance...",.3)
        self.update_enemy_fight_label("",.3)
        Clock.unschedule(self.update_bar)
        Clock.unschedule(self.enemy_update_bar)
        Selected_Dragon.agility= 0
        Selected_Dragon.intellect= 0
    ...
    def arena_exit(self, dt):
        new_game.unstoppable.stop()
        self.manager.current = 'begin_screen'
    ...
    def strength_dmg(self):
        if self.ids.strength_bar.value and Selected_Dragon.agility > 30:
            ...
            if self.enemy_agility_chance == 2:
                ...
                if int(self.ids.enemy_health_lbl.text) <= 0:
                    self.enemy_lose(.3)
                #vibrator.vibrate(time=2)
        else: 
